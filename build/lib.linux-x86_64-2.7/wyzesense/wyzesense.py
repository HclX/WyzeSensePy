...
class Sensor(object):
    def __init__(self, sensor_type, sensor_mac):
        self.Mac = sensor_mac
        self.Type = sensor_type

    def OnAlarm(timestamp, alarm_type, alarm_data):
        assert len(data) == 9

        seq = (alarm_data[6] << 8) | alarm_data[7]
        if seq == self.LastSeq:
            pass

        self.LastSeq = seq

        # Recording sensor type
        if self.Type != alarm_data[0]:
            assert self.Type == 0
            self.Type = alarm_data[0]

        self.BatteryLevel = alarm_data[2]
        self.State = alarm_data[5]
        self.SignalStrength = alarm_data[8]

        logging.info("[%s][%s] sensor state changed to %d", timestamp.isoformat(), self.Mac, self.State) 
...
class WyzeSense(object):
    ...
    def _OnSensorAlarm(self, pkt):
        if len(pkt.Payload) < 18:
            logging.info("Unknown alarm packet: %s", str_to_hex(pkt.Payload))
        else:
            ts, alarm_type, sensor_mac = struct.unpack_from(">QB8s", pkt.Payload)
            timestamp = datetime.datetime.fromtimestamp(ts/1000.0)
            alarm_data = [ord(x) for x in pkt.Payload[17:])
            s = self.GetSensor(sensor_mac)
            s.OnAlarm(timestamp, alarm_type, alarm_data)
    ...
    def Test(self):
        resp = self._DoSimpleCommand(Packet.Ch554Upgrade())
        if not resp:
            logging.debug("Ch554Upgrade timeout...")
...
try:
    ...
    while True:
        ...
        print("T for test")
        print("E to exit")
        action = raw_input("Action:").strip().lower()
        if action == "s":
